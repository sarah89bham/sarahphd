...
thisExp = data.ExperimentHandler(name=expName, version='',
    ...
    originPath='E:\\5_WORKING MEMORY\\OSPAN_yuhan\\OperationSpanTask_lastrun.py',
...
text_4 = visual.TextStim(win=win, name='text_4',
    text='In this task, you will do two things:\nFirst, you will see a math equation such as\n 3 + 4 = 7. Your job is to read the equation and verify it as either true or false. Press the LEFT arrow on the keyboard if the equation is correct and press the RIGHT arrow on the keyboard if it is incorrect.\n\nPress the SPACE bar to continue.',
    font='Arial',
...
text_6 = visual.TextStim(win=win, name='text_6',
    text='You will now begin practice trials.\n\nAs a reminder, if an equation is correct, press the LEFT arrow key. If an equation is incorrect, press the RIGHT arrow key.\n\nWhen the “recall” prompt appears, do your best to recall the letters that you were presented.\n\nPress the SPACE bar to continue.',
    font='Arial',
    pos=(0, 0), height=0.04, wrapWidth=None, ori=0, 
    color='white', colorSpace='rgb', opacity=1, 
    languageStyle='LTR',
...
for thisPractice in practice:
    ...
    practrials = data.TrialHandler(nReps=1, method='sequential', 
        extraInfo=expInfo, originPath=-1,
        trialList=data.importConditions(BlockNumber, selection='0:2'),
    ...
    textbox.reset()
    # keep track of which components have finished
    RecallComponents = [text_3, key_resp_3, key_resp_4, textbox]
    ...
    practice.addData('textbox.text',textbox.text)
...
# set up handler to look after randomisation of conditions etc
span2 = data.TrialHandler(nReps=0, method='sequential', 
    extraInfo=expInfo, originPath=-1,
...
for thisSpan2 in span2:
    ...
    trials2 = data.TrialHandler(nReps=1, method='sequential', 
        extraInfo=expInfo, originPath=-1,
        trialList=data.importConditions(BlockNumber, selection='2:4'),
    ...
    span2.addData('textbox.text',textbox.text)
    # the Routine "Recall" was not non-slip safe, so reset the non-slip timer
    routineTimer.reset()
    thisExp.nextEntry()
    
# completed 0 repeats of 'span2'
...
for thisSpan3 in span3:
    ...
    trials3 = data.TrialHandler(nReps=1, method='sequential', 
        extraInfo=expInfo, originPath=-1,
        trialList=data.importConditions(BlockNumber, selection='4:7'),
    ...
    span3.addData('textbox.text',textbox.text)
    # the Routine "Recall" was not non-slip safe, so reset the non-slip timer
    routineTimer.reset()
    thisExp.nextEntry()
...
for thisSpan4 in span4:
    ...
    trials4 = data.TrialHandler(nReps=1, method='sequential', 
        extraInfo=expInfo, originPath=-1,
        trialList=data.importConditions(BlockNumber, selection='7:11'),
    ...
    span4.addData('textbox.text',textbox.text)
    # the Routine "Recall" was not non-slip safe, so reset the non-slip timer
    routineTimer.reset()
    thisExp.nextEntry()
...
for thisSpan5 in span5:
    ...
    trials5 = data.TrialHandler(nReps=1, method='sequential', 
        extraInfo=expInfo, originPath=-1,
        trialList=data.importConditions(BlockNumber, selection='11:16'),
    ...
    span5.addData('textbox.text',textbox.text)
    # the Routine "Recall" was not non-slip safe, so reset the non-slip timer
    routineTimer.reset()
    thisExp.nextEntry()
...
for thisSpan6 in span6:
    ...
    trials6 = data.TrialHandler(nReps=1, method='sequential', 
        extraInfo=expInfo, originPath=-1,
        trialList=data.importConditions(BlockNumber, selection='16:22'),
    ...
    span6.addData('textbox.text',textbox.text)
    # the Routine "Recall" was not non-slip safe, so reset the non-slip timer
    routineTimer.reset()
    thisExp.nextEntry()
...
# set up handler to look after randomisation of conditions etc
span7 = data.TrialHandler(nReps=1, method='sequential', 
    extraInfo=expInfo, originPath=-1,
    trialList=data.importConditions('Blocks.xlsx', selection='0:3'),
    seed=None, name='span7')
thisExp.addLoop(span7)  # add the loop to the experiment
thisSpan7 = span7.trialList[0]  # so we can initialise stimuli with some values
# abbreviate parameter names if possible (e.g. rgb = thisSpan7.rgb)
if thisSpan7 != None:
    for paramName in thisSpan7:
        exec('{} = thisSpan7[paramName]'.format(paramName))

for thisSpan7 in span7:
    currentLoop = span7
    # abbreviate parameter names if possible (e.g. rgb = thisSpan7.rgb)
    if thisSpan7 != None:
        for paramName in thisSpan7:
    ...
    trials7 = data.TrialHandler(nReps=1, method='sequential', 
        extraInfo=expInfo, originPath=-1,
        trialList=data.importConditions(BlockNumber, selection='22:29'),
    ...
    if thisTrials7 != None:
        for paramName in thisTrials7:
            exec('{} = thisTrials7[paramName]'.format(paramName))
    ...
    for thisTrials7 in trials7:
        ...
        trials7.addData('key_resp_2.keys',key_resp_2.keys)
        trials7.addData('key_resp_2.corr', key_resp_2.corr)
        if key_resp_2.keys != None:  # we had a response
            trials7.addData('key_resp_2.rt', key_resp_2.rt)
        # the Routine "trial" was not non-slip safe, so reset the non-slip timer
        routineTimer.reset()
    ...
    if key_resp_3.keys in ['', [], None]:  # No response was made
        ...
        if str(CorRec).lower() == 'none':
           ...
           key_resp_3.corr = 0;  # failed to respond (incorrectly)
    # store data for span7 (TrialHandler)
    span7.addData('key_resp_3.keys',key_resp_3.keys)
    span7.addData('key_resp_3.corr', key_resp_3.corr)
    if key_resp_3.keys != None:  # we had a response
        span7.addData('key_resp_3.rt', key_resp_3.rt)
    # check responses
    if key_resp_4.keys in ['', [], None]:  # No response was made
        key_resp_4.keys = None
    span7.addData('key_resp_4.keys',key_resp_4.keys)
    if key_resp_4.keys != None:  # we had a response
        span7.addData('key_resp_4.rt', key_resp_4.rt)
    span7.addData('textbox.text',textbox.text)
    # the Routine "Recall" was not non-slip safe, so reset the non-slip timer
    routineTimer.reset()
    thisExp.nextEntry()
    
# completed 1 repeats of 'span7'
...
_key_resp_8_allKeys = []
# keep track of which components have finished
EndComponents = [text_9, key_resp_8]
